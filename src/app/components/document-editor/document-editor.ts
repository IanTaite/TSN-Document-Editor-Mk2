...
import { FormArray, FormGroup, FormBuilder, ReactiveFormsModule, FormControl } from '@angular/forms';
...
import {
  ...
  IImageLayer,
  IImageLayerStatic,
  IImageLayerText } from '../../image-definitions/interfaces';
...
@Component({
  ...
  standalone: true,
  imports: [JsonPipe, ReactiveFormsModule, VariantKeysEditorComponent, ImageLayerEditorComponent],
...
export class DocumentEditorComponent implements OnInit {
  ...
  private buildFormFromModel(model: IImageDefinition) {
    const form = this.fb.group({
      ...
      imageSizeTemplateId: this.fb.control<string>(model.imageSizeTemplateId ?? ''),
    ...
    const layerFormGroups = model.imageLayers
      ...
      .map((modelLayer: IImageLayer, modelLayerIndex: number) => {
        if (modelLayer.layerType === ELayerType.Static) {
          const layerFormGroup = this.convertStaticLayerToFormGroup(modelLayer as IImageLayerStatic);
          return layerFormGroup;
        }
        if (modelLayer.layerType === ELayerType.Text) {
          const layerFormGroup = this.convertTextLayerToFormGroup(modelLayer as IImageLayerText);
          return layerFormGroup;
        }
        return null;
      })
  ...
  private convertStaticLayerToFormGroup(input: IImageLayerStatic): FormGroup {
    const layerGroup = this.fb.group({
      ...
      components: this.fb.array(this.convertImageLayerComponentArrayToFormGroupArray(input.components)),
  ...
  private convertTextLayerToFormGroup(input: IImageLayerText): FormGroup {
    ...
    if (input.defaultImage === null || input.defaultImage.files === null) {
      ...
      forTheLoveOfGod = this.fb.group({
        files: this.fb.array(
          input.defaultImage.files.map(f => this.fb.group({
  ...
  private convertImageLayerComponentArrayToFormGroupArray(input: IComponentBase[]): FormGroup[] {
    ...
    input.forEach((component: IComponentBase) => {
      ...
      if (component.componentType === EComponentType.StaticAsset) {
        fg = this.convertStaticAssetComponentToFormGroup(component as IComponentStaticAsset);
      } else if (component.componentType === EComponentType.Directsmile) {
        fg = this.convertDirectSmileComponentToFormGroup(component as IComponentDirectSmile);
      }
      if (fg !== null) {
        result.push(fg);
      }
  ...
  private convertStaticAssetComponentToFormGroup(source: IComponentStaticAsset): FormGroup {
    return this.fb.group({
      paths: this.fb.array(source.paths?.map(path => this.fb.group({
  ...
  private convertDirectSmileComponentToFormGroup(source: IComponentDirectSmile): FormGroup {
    return this.fb.group({
      defaultText: this.fb.control<string>(source.defaultText ?? ''),
      textFormat: this.fb.control<string>(source.textFormat ?? ''),
      sets: this.fb.array(source.sets?.map(set => this.fb.group({
